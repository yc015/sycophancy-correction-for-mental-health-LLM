# YC: If we are going to use this linear probe, this very complicate code might be unnecessary
...
def train(probe, device, train_loader, optimizer, epoch, loss_func, 
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        # attention_masks = batch["attention_mask"].to(device)[0]
        target = batch["age"].long().cuda()
        if one_hot:
            target = torch.nn.functional.one_hot(target, **kwargs).float()
        optimizer.zero_grad()
        # with torch.no_grad():
        #     output = model(input_ids=input_ids.to("cuda"),
        #                    attention_mask=attention_masks.to("cuda"),
        #                    output_hidden_states=True,
        #                    return_dict=True)
        # act = output["hidden_states"][layer_num][:, -1].detach().clone().to(torch.float)
        # Compute the loss
...
def train_projected_gradient(probe, device, train_loader, optimizer, epoch, loss_func, 
          class_names=None, report=False, verbose_interval=5, layer_num=40,
          head=None, verbose=True, return_raw_outputs=False, one_hot=False, uncertainty=False, **kwargs,):
    """
    :param model: pytorch model (class:torch.nn.Module)
